...
std::map<uint16, NPCType> horse_types;

Horse::Horse(Client *_owner, const NPCType* npc_typedata, uint16 spell_id, const glm::vec4& position)
 : NPC(npc_typedata, nullptr, position, GravityBehavior::Water)
{
...
bool Horse::IsHorseSpell(uint16 spell_id) {
	...
	return(GetHorseType(spell_id).race != 0);
}
...
void Client::SummonHorse(uint16 spell_id) {
	if (GetHorseId() != 0) {
		Message_StringID(Chat::Red, ALREADY_ON_A_MOUNT);
		return;
	}
	if(!Horse::IsHorseSpell(spell_id)) {
		Message_StringID(Chat::Red, TARGET_HORSE_TO_MOUNT);
	...
	const NPCType npc_horse = Horse::GetHorseType(spell_id);
	memcpy(npc_type_copy, &npc_horse, sizeof(NPCType));

	// No Horse, lets get them one.

	auto horse = new Horse(this, npc_type_copy, spell_id, GetPosition());
