	...
	if (!(IsClient() && CastToClient()->GetGM()))
	{
		...
		if(spell_target && spell_target->IsClient() && spells[spell_id].targettype != ST_Self && !IsGroupSpell(spell_id))
		{
			...
			if(IsBeneficialSpell(spell_id))
			{
				...
				{
					bool can_get_experience =
						spell_target->CastToClient()->IsInLevelRange(caster->GetLevel(), caster->GetLevel2()) &&
						caster->IsInLevelRange(spell_target->CastToClient()->GetLevel(), spell_target->CastToClient()->GetLevel2());
